...
Para el mapeo de columnas usé un array de configuración en `config/proveedores.php`. Para un sistema grande lo más prolijo sería una clase por proveedor, pero para el alcance de este challenge me pareció excesivo. Teniendo todos los esquemas en un solo archivo es más fácil ver de un vistazo cómo viene cada proveedor y ajustarlo, en vez de repartir la lógica en muchos archivos casi vacíos. De todos modos no cierro esa puerta: el service consume el array de forma transparente, así que si en algún momento un proveedor necesita una transformación más compleja (más allá de mapear columnas), se puede pasar ese caso a una clase propia sin reescribir toda la importación.
...
- **Búsqueda indexada.** Si el catálogo crece a millones de registros, las consultas con `WHERE` van a empezar a degradarse. Ahí integraría un motor de búsqueda indexado por detrás, manteniendo el endpoint actual pero bajando los tiempos de respuesta.
